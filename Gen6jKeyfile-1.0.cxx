...
const std::string strVersion = "v1.0 init usable ver: Gen6jKeyfile-1.0";
const std::string strProgram = "Keyfile generator for ..6j..";
const std::string strAuthor = "Nethereal";
const std::string strDate = "26 Jul 2016";
const std::string strMsg = "be good to one another!";

char* fileName;
const uint8_t geneLength = 8; //length in bytes per gene
const uint16_t chromosomeLength = 256 * geneLength; //bytes total of 1 gene(8bytes) per char(256)

const uint64_t bytesInMB = 1048576;
const int MB_DEFAULT = 1;
const bool DFYWRITEON = false;


// utilfx
void motp(){
	std::cout << std::endl;
	std::cout << "    " << strProgram.c_str() << std::endl;
	std::cout << "    " << strVersion.c_str() << " - " << strDate.c_str() << std::endl;
	std::cout << "      by: " << strAuthor.c_str() << std::endl;
...
int main(int argc, char* argv[]) {
	...
	std::clock_t timerstart1;
	double timerduration1;
	std::clock_t timerstart2;
	double timerduration2;
	int timeRem1;
	int timeRem2;
	...
	std::vector<unsigned char> vecPostrandom;
	...
	try {
		...
		//vecPostrandom.resize(genomeLength, UCHAR_MAX);
								...
								if (genomeVfy6[lvl1a] == genomeVfy6[lvl1b]) {
									if (genomeVfy7[lvl1a] == genomeVfy7[lvl1b]) {
										if (genomeVfy8[lvl1a] == genomeVfy8[lvl1b]) {
											...
											for(uint8_t icnt = 0; icnt < geneLength; icnt++) { 
												genome[(lvl1a*8)+icnt] = (unsigned char)255;
												genome[(lvl1b*8)+icnt] = (unsigned char)255;
											}
											std::cout << "  =  + erased: FF:FF:FF:FF:FF:FF:FF:FF" << std::endl;
	...
	//uint64_t svcursize;
	//uint64_t randSelect;
	//std::vector<unsigned char>::iterator dvit;
	//for ( uint64_t vcnt = 0 ; vcnt < genomeLength; vcnt++ ){
	//	svcursize = vecGenome.size();
	//	std::cout << "Cursize for vecGenome: " << vecGenome.size() << "    vecPost: " << vecPostrandom.size() << std::endl;
	//	randSelect = arc4random_uniform(svcursize);
	//	dvit = vecPostrandom.begin();
	//	vecPostrandom.insert ( dvit , vecGenome.at(randSelect), vecGenome.at(randSelect) + 8 );
	//	vecGenome.erase(vecGenome.begin() + randSelect, vecGenome.begin() + randSelect + 8 );
	//}
	///
