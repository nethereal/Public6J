...
int main(int argc, char* argv[]) {
	...
	for (uint64_t countera = 0; countera < vfytop; countera++) { for (uint64_t counterb = (countera + 1); counterb < vfytop; counterb++) { totalvfys++; } }
	std::cout << "Finished calculating dupifys OK ..." << std::endl;
	std::cout << "Dupifying key table in memory..." << std::endl;
	uint64_t currelem = 0;
	bool matches = false;
	for (uint64_t lvl1a = 0; lvl1a < vfytop; lvl1a++) {
		std::cout << "Dupifying :      " << currelem << " of " << totalvfys << "\r";
	...
	if (matches) {
		...
		std::cout << "Dupifying key table in memory OK ...";
	}
