...
int main(int argc, char* argv[]) {
	...
	for (uint64_t genCnt = 0; genCnt < genomeLength; genCnt = genCnt + 8) { // Main memory object creation loop
		...
		genome[(genCnt + 0)] = newGene[0]; 		
		genome[(genCnt + 1)] = newGene[1]; 		
		genome[(genCnt + 2)] = newGene[2]; 		
		genome[(genCnt + 3)] = newGene[3]; 	
		genomeVfy1[(genCnt + 0)] = newGene[0];	
		genomeVfy2[(genCnt + 1)] = newGene[1];
		genomeVfy3[(genCnt + 2)] = newGene[2];
		genomeVfy4[(genCnt + 3)] = newGene[3];
		genome[(genCnt + 4)] = newGene[4]; 		
		genome[(genCnt + 5)] = newGene[5]; 		
		genome[(genCnt + 6)] = newGene[6]; 		
		genome[(genCnt + 7)] = newGene[7]; 		
