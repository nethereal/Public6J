...
const std::string strVersion = "2.5 NC";
...
int main(int argc, char* argv[]) {
	...
	char* arg2 = argv[2];
	for (unsigned int ix = 0; ix < strlen(arg2); ix++) {
		if (!isalnum(arg2[ix])) {
			std::cout << std::endl;
	...
	if (getFilesize(arg2) != genomeLength) {
		std::cout << std::endl;
	...
	uint64_t pElem = 0;
	for (uint64_t genCnt = 0; genCnt < genomeLength; genCnt = genCnt + 8) {
		if (genCnt >= pc[pElem]) {
